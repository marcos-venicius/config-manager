...
# 1. Create a function that builds your entire prompt
function __update_prompt() {
  # Grab the current context and store it in a local variable
  local k8s_context=$(kubectl config current-context 2>/dev/null)
  local k8s_context_format=""
  local git_branch=$(git branch --show-current 2>/dev/null | tr -d '\n')
  local git_branch_format=""

  if [[ -n $git_branch ]]; then
    git_branch_format="[$git_branch] "
  fi

  if [[ -n $k8s_context ]]; then
    k8s_context_format="($k8s_context) "
  fi
  
  # Build the PS1 string dynamically 
  if [ "$color_prompt" = yes ]; then
    PS1="$k8s_context_format$git_branch_format\[\033[01;34m\]\w\[\033[00m\]\n\$ "
  else
    PS1="$k8s_context_format$git_branch_format\w\n\$ "
  fi
}

# 2. Tell Bash to run this function right before displaying the prompt
...
# alias grep='grep -RIn --color=auto --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=dist --exclude-dir=obj --exclude-dir=bin'

alias today='date +'%Y-%m-%d''

function new_task_folder() {
  name=$(date +'%Y-%m-%d')
  path="$HOME/Documents/notes/$name"
...
export MCA_SEARCH_PATHS="$HOME/mca-libs/"

# machine-specific / sensitive settings (not versioned)
